...
$(document).ready(function() {
  // For static hosting, the components are already embedded in the HTML
  // Skip the Promise.all fetch calls and go straight to initialization
  console.log("Static site - components already loaded");
...
function initThemeSwitch() {
  ...
  const updateLogos = () => {
    ...
    if (lightMode) {
      ...
      // Update site logo to dark version for light mode
      siteLogos.attr("src", "/static/image/admire_logo.png");

      partnerLogos.each(function () {
        const $img = $(this);
        const src = $img.attr("src");
        if (!src.includes("-dark")) {
          // Replace the logo for dark mode with the new path
      ...
      // Reset site logo to normal version for dark mode
      siteLogos.attr("src", "/static/image/admire_logo.png");

      partnerLogos.each(function () {
        const $img = $(this);
        const src = $img.attr("src");
        // Reset partner logos when dark mode is not active
...
function initSubmitNewsletter() {
  ...
  console.log('Newsletter form handler initialized');
}

// initAnimateData function removed
...
function initDynamicText() {
  ...
  function typeWriter() {
    ...
    if (isDeleting) {
      dynamicElement.textContent = currentText.substring(0, currentCharIndex - 1);
      currentCharIndex--;
  ...
  console.log('Starting dynamic text...');
